...
<template>
    <AppLayout>
        <template #header>Recursos empregados na ocorrência</template>
        ...
        <div class="mb-4 flex flex-wrap items-center justify-between gap-2">
            ...
            <AppButton :as="Link" :href="route('create-resource', occurrence_id)" variant="success">
        ...
        <div class="divide-y divide-gray-100 overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-gray-950/5">
            <div v-for="resource in resources" :key="resource.id" class="flex flex-wrap items-center justify-between gap-3 px-5 py-3">
                <Link :href="route('show-resource', { occurrence_id, id: resource.id })" class="font-medium text-gray-900 hover:text-brand-blue">
                    {{ resource.who }}
                </Link>
                <div v-if="isAdmin()" class="flex items-center gap-2">
                    <AppButton :as="Link" :href="route('edit-resource', { occurrence_id, id: resource.id })" size="sm">
                        <Pencil class="h-3.5 w-3.5" /> Atualizar
                    </AppButton>
                    <AppButton type="button" variant="danger" size="sm" @click="ask(resource)">
